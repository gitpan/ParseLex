...
package C<Parse::Lex> crée indirectement une instance C<Parse::Token>
par token à reconnaître.  Les méthodes C<next()> ou C<isnext()> du
package C<Token> permettent d'interfacer l'analyseur lexical avec un
analyseur syntaxique de type récursif descendant. Pour un interfaçage
avec C<byacc> voir le package C<Parse::YYLex>.

L'inclusion de ce package est faite indirectement par un C<use
Parse::Lex>.
...
C<${$name}> est l'objet C<Parse::Token>.
...
définition. Elles permettent ou permettrons de reconnaître des
structures particulières comme, par exemple, les chaînes de caractères
entre guillemets, les commentaires C, etc. Voici les sous-classes que
j'ai l'intention de créer :

C<Parse::Token::Action> : permet d'insérer des expressions Perl entre
deux tokens de l'analyseur lexical.

C<Parse::Token::Simple> : pour définir les tokens qui ne nécessitent
pas le lecture de nouvelles données.

C<Parse::Token::Multiline> : pour définir des tokens pouvant
nécessiter la lecture de données complémentaires.

C<Parse::Token::Nested> : pour reconnaître des structures
imbriquées, tels que des expressions parenthésées.

C<Parse::Token::Delimited> : pour reconnaître, par exemple, des
chaînes de caractères entre guillemets.

Le nom de ces classes a valeur de proposition. Vous pouvez me faire
des suggestions.
...
C<$LEX_OFFSET> : position en nombre de caractères depuis le début de
l'analyse 

=item *

C<$LEX_POS> : position en nombre de caractères indiquant l'avancée de
l'analyse
...
=head2 Parse::Token::Delimited  - Non définie
...
et parfois envoyés des corrections de bugs (Stefan Becker, Rashid
Karimov, Larry W. Virden ...).
