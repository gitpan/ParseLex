...
C<Parse::Lex> - Générateur d'analyseurs lexicaux (Alpha 1.18).
...
antérieure, utilisez la sous-classe C<Parse::CLex>.

Les analyseurs générés par ces deux classes exploitent des stratégies
d'analyse différentes :

1. C<Parse::Lex> utilise C<pos()>, associé à "\G" pour
effectuer l'analyse,

2. C<Parse::CLex> effectue une analyse avec consommation des
tokens reconnus.

Attention ! Les analyseurs de la classe C<Parse::CLex> ne permettent
pas d'utiliser des expressions régulières avec ancrage.

Les lexèmes sont des objets de la classe C<Parse::Token>, livré avec
C<Parse::Lex>. La définition d'un lexème comporte ordinairement deux
arguments : un non symbolique (comme C<INTEGER>), suivi d'une
expression régulière. Si une fonction anonyme est donnée en troisième
argument, elle est exécutée lorsque le lexème est reconnu. Le retour
...
du lexème.  Pour chaque triplet, un objet de type C<Parse::Token> est
créé.
...
enregistrements. Si la définition du token comporte plusieurs
expressions régulières (et est placée dans un tableau anonyme)
C<Parse::Lex> lira autant d'enregistrement que nécessaire pour
reconnaître le token (voir la documentation de la classe
Parse::Token). Lorsque que le motif du début est trouvé, l'analyseur
en cherche la fin, et si besoin est, lit de nouveaux
...
Remarque. La fonction anonyme associée au token reçoit en argument
